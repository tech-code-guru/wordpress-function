...
$args=array( 'post_type'=> 'post', 'orderby'=> 'title', 'order'=> 'ASC', 'posts_per_page'=> 2, 'paged'=> $paged);
...
?></a></h2>
                                        <div class="clearfix post-meta">
                                            <p><span><i class="fa fa-user"></i> <?php echo the_author_meta( 'user_nicename', $post->post_author);
?> </span> <span><i class="fa fa-clock-o"></i> <?php echo get_the_date('d');
?> <?php echo get_the_date('M');
?> <?php echo get_the_date('Y');
?></span> <span><i class="fa fa-comment"></i> <?php echo $post->comment_count;
?> comments</span> <span><i class="fa fa-folder"></i> <?php $category=get_the_category($post->ID);
foreach ($category as $allCategory) {
    echo $firstCategory=$allCategory->name;
}
...
?></span></p>
                                        ...
                                        <div class="clearfix post_excerpt">
                                            <?php if($image[0]=="") {
    ?> <img src="http://localhost/wordpress/wp-content/uploads/2016/08/1311851327_preferences-contact-list.png" alt="" />
                                                <?php
}
...
?>
                                                        <p>
                                                            <?php echo substr($post->post_excerpt, 0, 300).'...';
?>
                                                        </p>
                                                        <?php the_field('user', $post->ID);
?>
                                                            </p>
                                                            </p>
                                        </div> <a href="<?php echo  get_permalink(); ?>">Continue Reading</a> </div>
                                </div>
                                <?php endwhile;
?>
                                    <?php $next_button=get_next_posts_link( 'Next Page', $the_query->max_num_pages);
if(!$next_button=="") {
    ?>
                                        <div class="next-button">
                                            <?php echo get_next_posts_link( 'Next Page', $the_query->max_num_pages);
    ?>
                                        </div>
                                        <?php
}
...
?>
                                                <?php $prev_button=get_previous_posts_link( 'Previous Page');
if(!$prev_button=="") {
    ?>
...
</section> <style> .next-button {
    background: url(images/nav_right.png) no-repeat scroll 130px 4px #ffd500;
    display: block;
